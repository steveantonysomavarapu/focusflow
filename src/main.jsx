...
function App() {
  ...
  useEffect(() => {
    if (!loading) {
      try {
        ...
        setError("Your browser could not save tasks locally.");
      }
    }
  }, [tasks, loading]);
  ...
  function submitTask(event) {
    ...
    if (editingId) {
      setTasks(
        tasks.map((task) =>
          task.id === editingId
            ? { ...task, ...form, title: form.title.trim() }
            : task,
        ),
      );
      setEditingId(null);
      ...
      setTasks([
        { ...form, title: form.title.trim(), id: newTaskId(), done: false },
        ...tasks,
      ]);
    }
    setForm(emptyForm);
  ...
  function remove(id) {
    ...
    if (
      ...
      setTasks(tasks.filter((candidate) => candidate.id !== id));
  }
    ...
    <main className="app-shell">
      ...
      <section className="hero">
        <div className="progress">
          ...
          <small>of tasks completed</small>
          <div className="bar">
            <i
              style={{
      ...
      <section className="stats" aria-label="Task statistics">
        ...
        <article>
          ...
          <div>
            <b>
              {
                tasks.filter((task) => task.priority === "High" && !task.done)
                  .length
              }
            </b>
      ...
      <section className="workspace">
        <div className="panel add-panel">
          <h3>{editingId ? "Edit task" : "Add a task"}</h3>
          <form onSubmit={submitTask}>
            ...
            <div className="form-actions">
              <button type="submit">
                {editingId ? "Save changes" : "Add to my list"} <span>→</span>
              </button>
              {editingId && (
                <button
                  type="button"
        ...
        <div className="panel list-panel">
          ...
          <nav aria-label="Filter tasks by category">
